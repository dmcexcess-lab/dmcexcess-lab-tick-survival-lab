...
**Progress — same-tick walk arbitration slice complete 2026-09-25.** Canonical movement now admits actor-occupied walk targets to timestamp arbitration, evaluates all due movers against one pre-resolution occupancy state, resolves same-destination conflicts without callback/actor-ID initiative, and allows reciprocal/vacating actor moves to commit atomically through the existing placement batch. The provisional all-fail same-destination rule has now been refined: a unique highest derived physical-stat score wins the exclusive destination; an exact/unknown top tie remains a stalemate. The current score uses existing condition-adjusted carry capacity, current load, HP, stance and movement intent rather than inventing a Strength stat. Walk becomes committed at its final `movement.commit` timestamp so later same-tick interruption cannot erase an already-due placement. Phase 2 remains open for shove-vs-move/displacement arbitration, cross-system death/movement closure, mob force, canonical fear, zombie callback/perception cost, presentation coherence and crowded-fight performance.
