...
**Progress — causal movement/shove transition slices complete 2026-09-25.** Canonical movement admits actor-occupied walk targets to timestamp arbitration, evaluates due trajectories from one stable incoming occupancy state, distinguishes conditional origin release from destination arrival, and resolves exclusive claims without callback/actor-ID initiative. Same-destination movement uses frozen canonical physical scores; compatible release chains advance atomically; reciprocal ordinary walks are head-on edge conflicts. Combat shove now seals a forced target trajectory with pre-damage physical force/resistance and submits it into this same late spatial batch. A target's own committed move can oppose or align with shove; equal opposing shoves leave the target in place. Combat holds lethal Health consequence publication through the spatial flush, so an already-earned shove/move survives same-tick lethal damage and the corpse inherits the resolved outgoing position. Phase 2 remains open for same-direction crowd-force aggregation, push-chain/pressure consequences, canonical fear, zombie callback/perception cost, presentation coherence and crowded-fight performance.
