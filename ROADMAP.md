...
**Progress — same-tick walk arbitration slices complete 2026-09-25.** Canonical movement now admits actor-occupied walk targets to timestamp arbitration, evaluates all due movers against one pre-resolution occupancy state, resolves same-destination conflicts without callback/actor-ID initiative, and distinguishes origin release from destination arrival. A unique highest derived physical-stat score wins an exclusive same-destination claim; an exact/unknown top tie remains a stalemate. A one-direction vacating chain may advance atomically into cells released ahead of it, but reciprocal ordinary walks across the same edge are a head-on physical conflict and both fail rather than swapping through one another. The current score uses existing condition-adjusted carry capacity, current load, HP, stance and movement intent rather than inventing a Strength stat. Walk becomes committed at its final `movement.commit` timestamp so later same-tick interruption cannot erase an already-due placement. Phase 2 remains open for shove-vs-move/displacement arbitration, cross-system death/movement closure, mob force, canonical fear, zombie callback/perception cost, presentation coherence and crowded-fight performance.
