...
**Progress — same-tick walk arbitration slice complete 2026-09-25.** Canonical movement now admits actor-occupied walk targets to timestamp arbitration, evaluates all due movers against one pre-resolution occupancy state, fails every claimant in a same-destination conflict instead of picking a sorted winner, and allows reciprocal/vacating actor moves to commit atomically through the existing placement batch. Walk becomes committed at its final `movement.commit` timestamp so later same-tick interruption cannot erase an already-due placement. Phase 2 remains open for shove-vs-move/displacement arbitration, cross-system death/movement closure, mob force, canonical fear, zombie callback/perception cost, presentation coherence and crowded-fight performance.
