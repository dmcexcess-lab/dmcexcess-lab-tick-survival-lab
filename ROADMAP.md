...
**Progress — causal movement/shove/mob-force core complete 2026-09-25.** Canonical movement and shove resolve from one stable incoming physical state with conditional releases, atomic placement, frozen contests and no hidden initiative. Ordinary committed movement into an occupied body contributes locomotion contact force automatically, so simple zombie pursuit generates real crowd pressure without crowd AI. Parallel force adds, opposite force subtracts, and residual pressure propagates through a bounded multi-body chain while each body consumes frozen resistance. Force exhaustion stops a chain naturally; static geometry stops it absolutely; visited-path plus depth/pass bounds prevent cyclic/infinite propagation. This closes the release-level mob-force architecture. Knockdown/crush and fortification damage are downstream consequence mechanics, not blockers to closing crowd pressure.

**Progress — canonical fear complete 2026-09-25.** Existing `CALM` is the only fear truth. Visible infected threat, recognized alarming sound, injury shock and resolved crowd pressure now feed one same-timestamp fear-pressure owner, capped at 20 Calm loss per tick. Uneasy is warning-only; Afraid/Terrified explicitly slow deliberate actions, increase Fatigue gain and reduce hold/bracing resistance while leaving movement, shove, snap fire, max Health, carry, damage and player choice intact. Visual fear is distance-banded, diminishing and encounter-aware rather than repeatedly charging on perception refresh. Calm recovers analytically toward neutral over hours of game time.

**Progress — eight-infected callback/perception performance bounded 2026-09-25.** The production cohort remains eight simple independent infected with no scheduler or horde brain. Behavior now refreshes Perception only when observer/acquisition truth is stale; lighting-only acquisition refresh reuses cached geometric LOS; player + active infected share one bounded physical-light field; and that field is warmed during activation/loading rather than on the first real input. A focused two-decision production run measured first/second action-start callbacks at 9.874/9.580 ms, infected behavior at 8.984/8.905 ms per decision, and zero infected LOS geometry rebuilds. The same route immediately before warming measured a 113.442 ms first callback, isolating the cold field as the first-input hitch. Phase 2 remains open for coherent overlapping consequence presentation and crowded-fight/Safari acceptance; the full-turn route still contains cost outside infected callback/perception and must not be mislabeled solved.
...
### Phase 2 crowded-fight acceptance/performance closure — 2026-09-25

A fresh production-scene verifier now exercises the real touch-first Button -> semantic intent -> production controller path with the active eight resident-backed infected arranged into a bounded valid crowd solely for acceptance setup. After that setup, ordinary production WHEN, perception, infected behavior, movement, combat, Health/death, mob pressure and consequence presentation own every result.

The accepted route completed 2 turn actions and 3 strike actions across 27 world ticks with 5/5 input-lock cycles returning only at legitimate decision pauses. It produced real physical contact and overlapping same-timestamp consequence presentation while keeping all eight infected active. The focused run on head `51350037bb890efc4e2684027142a939f0d4687a` is GitHub Actions run `36208557288` — SUCCESS.

Measured CI route cost was 1,937,370 usec across the five accepted actions, max 477,635 usec for one accepted action. Existing measured phases reported behavior evaluation 52,684 usec, infected perception 81,204 usec, player perception 63,629 usec and active-cohort sync 20,552 usec. Global telemetry additionally reported lighting rebuild 262,873 usec, lighting geometry rebuild 151,494 usec, lighting draw 163,862 usec, perception recompute 144,833 usec and infected behavior evaluation 52,684 usec. These timings overlap and must not be summed as exclusive phases. Most route time remains outside the existing exclusive counters; therefore this pass does not justify a speculative performance-architecture rewrite. Lighting is the largest currently measured bounded phase, but it is not sufficient by itself to explain the route.

The shared mobile control path is now CI-proven without a desktop-only substitute. A Linux GitHub Actions runner cannot prove actual Safari/WebKit browser behavior. Phase 2 technical combat work is closed; the one remaining Phase 2 release-acceptance blocker is a real Safari browser pass on the deployed Pages build. That pass should verify touch activation, viewport fit, decision-pause/input-lock behavior and readable consequence moments. A Safari-specific defect, if observed, should be repaired narrowly; otherwise Phase 2 is accepted and work advances to Phase 3.
