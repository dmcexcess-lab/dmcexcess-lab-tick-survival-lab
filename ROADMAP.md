...
**Progress — causal movement/shove/mob-force core complete 2026-09-25.** Canonical movement and shove resolve from one stable incoming physical state with conditional releases, atomic placement, frozen contests and no hidden initiative. Ordinary committed movement into an occupied body now contributes locomotion contact force automatically, so simple zombie pursuit generates real crowd pressure without crowd AI. Parallel force adds, opposite force subtracts, and residual pressure propagates through a bounded multi-body chain while each body consumes frozen resistance. Force exhaustion stops a chain naturally; static geometry stops it absolutely; visited-path plus depth/pass bounds prevent cyclic/infinite propagation. This closes the release-level mob-force architecture. Knockdown/crush and fortification damage are downstream consequence mechanics, not blockers to closing crowd pressure. Phase 2 remains open for canonical fear, zombie callback/perception cost, presentation coherence and crowded-fight performance.
