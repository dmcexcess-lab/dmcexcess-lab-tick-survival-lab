extends RefCounted
class_name GlobalWastewaterInfrastructureValidator

const QueryClass = preload("res://scripts/generation/world/GlobalWastewaterInfrastructureQuery.gd")
...
var _query: GlobalWastewaterInfrastructureQuery
...
func validate(request: GlobalWorldGenerationRequest, plan: GeneratedGlobalWorldPlan) -> Dictionary:
    ...
    if request == null or not request.is_valid() or plan == null:
        return {"ok": false, "failures": ["invalid_global_wastewater_validation_input"]}
    if plan.wastewater_services.is_empty():
        failures.append("global_wastewater_services_missing")
    if plan.wastewater_nodes.is_empty():
        failures.append("global_wastewater_nodes_missing")
    if plan.wastewater_segments.is_empty():
        failures.append("global_wastewater_segments_missing")
    ...
    for service: Dictionary in plan.wastewater_services:
        _claim_wastewater_id(all_ids, String(service.get("id", "")), failures)
        ...
        if settlement_id.is_empty() or not known_settlements.has(settlement_id):
            failures.append("global_wastewater_service_settlement_missing")
            continue
        if service_by_settlement.has(settlement_id):
            failures.append("global_wastewater_service_duplicate")
        service_by_settlement[settlement_id] = true
        var mode: StringName = StringName(service.get("service_mode", &""))
        var disposal_type: StringName = StringName(service.get("disposal_type", &""))
        var network_id: String = String(service.get("network_id", ""))
        var separation_policy: StringName = StringName(service.get("separation_policy", &""))
        if settlement_id == SMALLTOWN_ID:
            municipal_count += 1
            if mode != &"municipal" or disposal_type != &"municipal_treatment" or network_id != NETWORK_ID or separation_policy != &"":
                failures.append("global_wastewater_smalltown_service_invalid")
        elif expected_rural.has(settlement_id):
            if mode != &"decentralized_septic" or disposal_type != &"onsite_septic":
                failures.append("global_wastewater_rural_service_invalid")
            if not network_id.is_empty():
                failures.append("global_wastewater_rural_claims_network")
            if separation_policy != &"potable_source_clearance_required":
                failures.append("global_wastewater_rural_separation_policy_missing")
        else:
            failures.append("global_wastewater_current_settlement_unsupported")

    if plan.wastewater_services.size() != known_settlements.size() or service_by_settlement.size() != known_settlements.size():
        failures.append("global_wastewater_service_count_invalid")
    if municipal_count != 1:
        failures.append("global_wastewater_municipal_count_invalid")
    for settlement_id_value: Variant in known_settlements.keys():
        if not service_by_settlement.has(settlement_id_value):
            failures.append("global_wastewater_settlement_service_missing")

    var smalltown_site: Dictionary = _site_for_settlement(plan.area_sites, SMALLTOWN_ID)
    if smalltown_site.is_empty():
        failures.append("global_wastewater_smalltown_site_missing")
    ...
    var node_counts: Dictionary = {&"settlement_collection": 0, &"treatment_disposal": 0}
    var node_cells: Dictionary = {}
    for node: Dictionary in plan.wastewater_nodes:
        _claim_wastewater_id(all_ids, String(node.get("id", "")), failures)
        if String(node.get("network_id", "")) != NETWORK_ID:
            failures.append("global_wastewater_node_network_invalid")
        if String(node.get("settlement_id", "")) != SMALLTOWN_ID:
            failures.append("global_wastewater_node_not_smalltown")
        var cell: Vector2i = node.get("cell", INVALID_CELL)
        if cell == INVALID_CELL or not plan.bounds.has_point(cell):
            failures.append("global_wastewater_node_out_of_bounds")
        if not site_bounds.has_point(cell):
            failures.append("global_wastewater_node_outside_smalltown_site")
        if not _point_on_any_road(cell, plan.road_segments):
            failures.append("global_wastewater_node_off_major_road")
        if _is_boundary_cell(plan.bounds, cell):
            failures.append("global_wastewater_node_on_world_boundary")
        var kind: StringName = StringName(node.get("kind", &""))
        if not node_counts.has(kind):
            failures.append("global_wastewater_node_kind_invalid")
            continue
        node_counts[kind] = int(node_counts[kind]) + 1
        node_cells[kind] = cell

    if plan.wastewater_nodes.size() != 2:
        failures.append("global_wastewater_node_count_invalid")
    for count_value: Variant in node_counts.values():
        if int(count_value) != 1:
            failures.append("global_wastewater_required_node_count_invalid")
    var collection_cell: Vector2i = node_cells.get(&"settlement_collection", INVALID_CELL)
    var treatment_cell: Vector2i = node_cells.get(&"treatment_disposal", INVALID_CELL)
    if collection_cell != smalltown_center:
        failures.append("global_wastewater_collection_location_mismatch")
    if treatment_cell == INVALID_CELL or treatment_cell == collection_cell:
        failures.append("global_wastewater_treatment_location_invalid")
    for water_node: Dictionary in plan.water_nodes:
        if treatment_cell != INVALID_CELL and water_node.get("cell", INVALID_CELL) == treatment_cell:
            failures.append("global_wastewater_treatment_overlaps_water_node")

    if plan.wastewater_segments.size() != 1:
        failures.append("global_wastewater_segment_count_invalid")
    var edge_keys: Dictionary = {}
    for segment: Dictionary in plan.wastewater_segments:
        _claim_wastewater_id(all_ids, String(segment.get("id", "")), failures)
        if String(segment.get("network_id", "")) != NETWORK_ID:
            failures.append("global_wastewater_segment_network_invalid")
        if StringName(segment.get("wastewater_class", &"")) != &"municipal_collection_trunk":
            failures.append("global_wastewater_segment_class_invalid")
        ...
        if start == INVALID_CELL or finish == INVALID_CELL or start == finish or (start.x != finish.x and start.y != finish.y):
            failures.append("global_wastewater_segment_not_cardinal")
            continue
        if not plan.bounds.has_point(start) or not plan.bounds.has_point(finish):
            failures.append("global_wastewater_segment_out_of_bounds")
        if not site_bounds.has_point(start) or not site_bounds.has_point(finish):
            failures.append("global_wastewater_segment_outside_smalltown_site")
        if _is_boundary_cell(plan.bounds, start) or _is_boundary_cell(plan.bounds, finish):
            failures.append("global_wastewater_segment_reaches_world_boundary")
        var edge_key: String = _edge_key(start, finish)
        if edge_keys.has(edge_key):
            failures.append("global_wastewater_segment_duplicate")
        ...
        if source_road.is_empty():
            failures.append("global_wastewater_source_road_missing")
        else:
            if String(source_road.get("route_id", "")) != source_route_id:
                failures.append("global_wastewater_source_route_mismatch")
            if not _query.segment_contained_in(segment, source_road):
                failures.append("global_wastewater_segment_off_source_road")
        if _segment_intersects_ridge(segment, plan.geography_cells):
            failures.append("global_wastewater_crosses_ridge")
        for water_segment: Dictionary in plan.water_segments:
            if _query.segments_overlap_positive_length(segment, water_segment):
                failures.append("global_wastewater_segment_overlaps_water_trunk")
                break
        if collection_cell != INVALID_CELL and treatment_cell != INVALID_CELL:
            if _edge_key(start, finish) != _edge_key(collection_cell, treatment_cell):
                failures.append("global_wastewater_collection_treatment_link_invalid")
...
func _claim_existing_plan_ids(plan: GeneratedGlobalWorldPlan, ids: Dictionary) -> void:
    ...
    for water_service: Dictionary in plan.water_services:
        ids[String(water_service.get("id", ""))] = true
    for water_node: Dictionary in plan.water_nodes:
        ids[String(water_node.get("id", ""))] = true
    for water_segment: Dictionary in plan.water_segments:
        ids[String(water_segment.get("id", ""))] = true
    for site: Dictionary in plan.area_sites:
        ids[String(site.get("id", ""))] = true

func _claim_wastewater_id(ids: Dictionary, id: String, failures: Array[String]) -> void:
    if id.strip_edges().is_empty():
        failures.append("global_wastewater_id_missing")
        return
    if ids.has(id):
        failures.append("global_wastewater_id_duplicate")
        return
    ids[id] = true
