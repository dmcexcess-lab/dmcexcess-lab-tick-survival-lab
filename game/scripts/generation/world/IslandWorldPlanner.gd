...
func _clip_collinear_segment(segment: Dictionary, road: Dictionary) -> Dictionary:
    ...
    if a.y == b.y and r0.y == r1.y and a.y == r0.y:
        ...
        if hi <= lo:
            return {}
        return {
            "start": Vector2i(lo, a.y) if a.x <= b.x else Vector2i(hi, a.y),
            "end": Vector2i(hi, a.y) if a.x <= b.x else Vector2i(lo, a.y),
        }
    if a.x == b.x and r0.x == r1.x and a.x == r0.x:
        ...
        if hi_y <= lo_y:
            return {}
        return {
            "start": Vector2i(a.x, lo_y) if a.y <= b.y else Vector2i(a.x, hi_y),
            "end": Vector2i(a.x, hi_y) if a.y <= b.y else Vector2i(a.x, lo_y),
        }
    return {}

func _build_regions(base_regions: Array[Dictionary], world_bounds: Rect2i) -> Array[Dictionary]:
    var result: Array[Dictionary] = []
