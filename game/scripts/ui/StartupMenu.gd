...
func _ready() -> void:
    ...
    _status_label.text = "Menu ready. Loading game systems…"
...
func _begin_gameplay_preload() -> void:
    ...
    if error != OK:
        ...
        _status_label.text = "Game preload unavailable. NEW GAME will load it directly."
        _progress_bar.value = PRELOAD_PROGRESS_START
        return
...
func _process(_delta: float) -> void:
    ...
    match status:
        ResourceLoader.THREAD_LOAD_IN_PROGRESS:
            _status_label.text = "Loading gameplay systems…"
        ResourceLoader.THREAD_LOAD_LOADED:
            ...
            if _gameplay_scene == null:
                _preload_failed = true
                _status_label.text = "Preload finished without a playable scene. NEW GAME will retry."
                return
            _progress_bar.value = PRELOAD_PROGRESS_START + PRELOAD_PROGRESS_SPAN
            _status_label.text = "Game systems ready. World generation starts with NEW GAME."
            set_process(false)
        ResourceLoader.THREAD_LOAD_FAILED, ResourceLoader.THREAD_LOAD_INVALID_RESOURCE:
            _preload_failed = true
            _status_label.text = "Game preload failed. NEW GAME will retry directly."
            set_process(false)
...
func _on_new_game_pressed() -> void:
    ...
    _status_label.text = "Finishing gameplay load…"
    ...
    if packed_scene == null:
        ...
        _status_label.text = "Unable to load the gameplay scene."
        _progress_bar.value = 0.0
        return

    _status_label.text = "Generating island and activating the starting region…"
    ...
    if game == null:
        _launching = false
        _new_game_button.disabled = false
        _status_label.text = "Unable to create the gameplay scene."
        return
