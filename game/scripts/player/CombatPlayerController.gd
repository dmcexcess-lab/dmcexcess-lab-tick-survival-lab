...
func _init(combat: CombatActionService = null, kernel: TickKernel = null, actor_id: String = "") -> void:
    _combat = combat
    _kernel = kernel
...
func is_ready() -> bool:
    return _combat != null and _combat.is_ready() and _kernel != null and not _actor_id.is_empty()

func is_busy() -> bool:
    return _busy

func submit_intent(intent: StringName) -> void:
    if not is_ready() or intent != Intents.COMBAT_FORWARD or _busy or not _kernel.is_decision_paused():
        return
    var result: Dictionary = _combat.request_forward(_actor_id)
    if not bool(result.get("accepted", false)):
        action_resolved.emit(intent, false, String(result.get("reason", "combat_rejected")), _kernel.world_tick())
        return
    var serial: int = int(result.get("action_serial", 0))
    if serial <= 0:
        action_resolved.emit(intent, false, "combat_action_invalid", _kernel.world_tick())
        return
...
func _on_action_finished(action: TimedAction) -> void:
    if action == null or action.serial != _active_serial or action.actor_id != _actor_id:
        return
    _outcomes[action.serial] = {
        "success": action.status == Rules.ActionStatus.COMPLETED,
