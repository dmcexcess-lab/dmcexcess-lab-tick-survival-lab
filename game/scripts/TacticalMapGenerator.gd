...
# Extracted from First Fire's FFTacticalEnvironments.gd as the clean bootstrap
# for Tick Survival Lab. This module owns physical authored map specifications
# only. It has no camp, expedition, combat, UI, or save dependencies.

const BOARD_W := 20
const BOARD_H := 18

const CATALOG := {
    "back_alley": {"name": "Back Alley", "theme": "alley", "variants": 2},
    "gas_station": {"name": "Gas Station", "theme": "gas", "variants": 2},
    "house": {"name": "Residential House", "theme": "house", "variants": 2},
    "apartment": {"name": "Apartment", "theme": "apartment", "variants": 2},
    "corner_store": {"name": "Corner Store", "theme": "store", "variants": 2},
    "warehouse_yard": {"name": "Warehouse Yard", "theme": "industrial", "variants": 2},
    "drainage_wash": {"name": "Drainage Wash", "theme": "wash", "variants": 2},
}
...
static func display_name(environment_id: String) -> String:
    ...
    return str(data.get("name", "Unknown Place"))
...
static func build_layout(environment_id: String, raw_variant: int) -> Dictionary:
    var variant: int = posmod(raw_variant, variant_count(environment_id))
...
static func validate_all() -> Dictionary:
    var failures: Array = []
    for environment_id_value in all_ids():
        var environment_id := str(environment_id_value)
        for variant in range(variant_count(environment_id)):
            var spec: Dictionary = build_layout(environment_id, variant)
            if not validate_layout(spec):
                failures.append("%s:%d" % [environment_id, variant])
...
static func validate_layout(spec: Dictionary) -> bool:
    ...
    for exit_cell_value in exits:
        var exit_cell: Vector2i = exit_cell_value
    ...
    for exit_cell_value in exits:
        if not seen.has(exit_cell_value):
            return false
    return true

static func ground_at(spec: Dictionary, p: Vector2i) -> String:
    var result := str(spec.get("default_ground", "asphalt"))
...
static func _spec(default_ground_kind: String, player_spawn: Vector2i, exits: Array) -> Dictionary:
    return {
...
static func _back_alley(variant: int) -> Dictionary:
    ...
    if variant == 1: exits.append(Vector2i(10, 1))
    var spec: Dictionary = _spec("asphalt", Vector2i(9, 14), exits)
    ...
    for p in [Vector2i(5,5), Vector2i(6,5), Vector2i(13,11), Vector2i(14,11)]:
        _obstacle(spec, p, "dumpster")
...
static func _gas_station(variant: int) -> Dictionary:
    ...
    if variant == 1: exits.append(Vector2i(1, 11))
    var spec: Dictionary = _spec("asphalt", Vector2i(4, 14), exits)
...
static func _house(variant: int) -> Dictionary:
    ...
    if variant == 1: exits.append(Vector2i(15, 1))
    var spec: Dictionary = _spec("grass", Vector2i(10, 12), exits)
    ...
    if variant == 1: _door(spec, Vector2i(15, 2), false)
    _wall_x(spec, 8, 6, 16)
    _door(spec, Vector2i(9, 8), false)
...
static func _apartment(variant: int) -> Dictionary:
    ...
    if variant == 1: exits.append(Vector2i(17, 8))
    var spec: Dictionary = _spec("sidewalk", Vector2i(9, 13), exits)
    ...
    if variant == 1: _door(spec, Vector2i(17, 8), false)
    _wall_y(spec, 7, 3, 13)
    ...
    if variant == 1: _light(spec, Vector2i(14, 12), "warm")
    return spec

static func _corner_store(variant: int) -> Dictionary:
    var exits: Array = [Vector2i(10, 15)]
    if variant == 1: exits.append(Vector2i(18, 7))
    var spec: Dictionary = _spec("sidewalk", Vector2i(10, 12), exits)
    ...
    if variant == 1: _door(spec, Vector2i(18, 7), false)
    for x in [6, 7, 13, 14, 15]: _window(spec, Vector2i(x, 13))
    for y in range(5, 11):
        if y not in [7, 9]:
...
static func _warehouse_yard(variant: int) -> Dictionary:
    var exits: Array = [Vector2i(2, 16), Vector2i(17, 16)]
    if variant == 1: exits.append(Vector2i(1, 8))
    var spec: Dictionary = _spec("concrete", Vector2i(4, 14), exits)
...
static func _drainage_wash(variant: int) -> Dictionary:
    var exits: Array = [Vector2i(9, 16)]
    if variant == 1: exits.append(Vector2i(10, 1))
    var spec: Dictionary = _spec("dirt", Vector2i(9, 14), exits)
