...
func configure(renderer: TacticalRendererStack, world_view: Node2D, door_pointer: DoorPointerInputAdapter, area_bounds: Rect2i, window_size: Vector2i, cell_pixels: float, initial_origin: Vector2i, base_pixel_origin: Vector2 = Vector2.ZERO) -> bool:
    ...
    if area_bounds.size.x <= 0 or area_bounds.size.y <= 0 or window_size.x <= 0 or window_size.y <= 0 or cell_pixels <= 0.0: return false
    if window_size.x > area_bounds.size.x or window_size.y > area_bounds.size.y: return false
    _renderer = renderer; _world_view = world_view; _door_pointer = door_pointer
    _area_bounds = area_bounds; _window_size = window_size; _cell_pixels = cell_pixels; _base_pixel_origin = base_pixel_origin; _configured = true
...
func attach_camera(camera_controller: TacticalCameraController) -> bool:
    if not _configured or camera_controller == null or not camera_controller.is_configured(): return false
    if _camera_controller != null:
        var old_callable := Callable(self, "_on_camera_presentation_changed")
        if _camera_controller.presentation_changed.is_connected(old_callable): _camera_controller.presentation_changed.disconnect(old_callable)
    _camera_controller = camera_controller
    var changed_callable := Callable(self, "_on_camera_presentation_changed")
    if not _camera_controller.presentation_changed.is_connected(changed_callable): _camera_controller.presentation_changed.connect(changed_callable)
    if not _camera_controller.set_render_window(_render_origin, _cell_pixels, _world_view): return false
    _on_camera_presentation_changed(_camera_controller.presentation_snapshot())
    return true

func is_configured() -> bool: return _configured
func render_origin() -> Vector2i: return _render_origin
func render_size() -> Vector2i: return _window_size
func area_bounds() -> Rect2i: return _area_bounds
func cell_pixels() -> float: return _cell_pixels
...
func _apply_window(origin: Vector2i, notify_camera: bool) -> bool:
    ...
    if ok: ok = _door_pointer.configure(_world_view.position, _render_origin, _window_size, _cell_pixels)
    if ok and notify_camera and _camera_controller != null and _camera_controller.is_configured(): ok = _camera_controller.set_render_window(_render_origin, _cell_pixels, _world_view)
    _shifting = false
    if notify_camera:
        PerformanceTelemetry.record_timing(&"render_window_shift", Time.get_ticks_usec() - started)
...
func _clamp_origin(origin: Vector2i) -> Vector2i:
    ...
    return Vector2i(clampi(origin.x, _area_bounds.position.x, max_origin.x), clampi(origin.y, _area_bounds.position.y, max_origin.y))
