extends RefCounted
class_name TacticalSound

# Tick Survival Lab adaptation of First Fire's reusable tactical sound helpers.
# This owns sound interpretation helpers only. Emission, propagation, occlusion,
# persistence, and AI response belong to future world/actor systems.
...
static func ambient_profile(theme: String, time_of_day: String, power_on: bool, rng: RandomNumberGenerator) -> Dictionary:
    ...
    match theme:
        ...
        "house": options.append({"label": "house creak", "intensity": 17})
        "apartment": options.append({"label": "pipe knock", "intensity": 20})
        "store": options.append({"label": "shelf tick", "intensity": 16})
        "industrial": options.append({"label": "sheet metal", "intensity": 23})
        "wash":
            options.append({"label": "wind", "intensity": 19})
            options.append({"label": "loose gravel", "intensity": 18})
        "gas": options.append({"label": "sign creak", "intensity": 18})
