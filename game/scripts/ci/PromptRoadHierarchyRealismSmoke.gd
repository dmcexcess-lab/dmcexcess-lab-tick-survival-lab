...
func _test_hierarchy(plan: Variant) -> void:
    ...
    var major_gateway_routes: int = 0
    var ordinary_gateway_routes: int = 0
    var rural_tree_routes: int = 0
    var rural_local_routes: int = 0

    for road: Dictionary in plan.road_segments:
        var road_type: StringName = StringName(road.get("road_type", &""))
        _check(type_counts.has(road_type), "road type is one of the four hierarchy classes")
        if type_counts.has(road_type):
            type_counts[road_type] = int(type_counts[road_type]) + 1
        _check(_surface_contract_is_valid(road), "road surface/lane contract matches road type: %s" % String(road.get("road_id", "")))

    for route_value: Variant in routes.keys():
        var route_id: String = String(route_value)
        var segments: Array = routes[route_value]
        _check(not segments.is_empty(), "route has physical segments: %s" % route_id)
        if segments.is_empty():
            continue
        _check(_route_is_contiguous(segments), "route remains geometrically contiguous: %s" % route_id)

        if route_id.begins_with("route.island.gateway."):
            gateway_routes += 1
            var side: StringName = StringName(route_id.substr("route.island.gateway.".length()))
            _check(_all_class(segments, &"primary"), "gateway remains a primary route: %s" % route_id)
            if freeway_sides.has(side):
                major_gateway_routes += 1
                _check(_contains_type(segments, &"four_lane"), "major gateway has a four-lane freeway trunk: %s" % route_id)
                _check(_contains_type(segments, &"two_lane"), "major gateway narrows before its settlement: %s" % route_id)
                _check(StringName(segments[0].get("road_type", &"")) == &"two_lane", "major gateway begins as two-lane at its settlement: %s" % route_id)
                _check(_single_forward_freeway_transition(segments), "major gateway transitions once from town approach to freeway: %s" % route_id)
            else:
                ordinary_gateway_routes += 1
                _check(_all_type(segments, &"two_lane"), "non-freeway gateway remains two-lane: %s" % route_id)
            continue

        var road_class: StringName = StringName(segments[0].get("road_class", &""))
        if road_class == &"primary":
            _check(_all_type(segments, &"two_lane"), "town/crossroads routes are paved two-lane: %s" % route_id)
        elif route_id.begins_with("route.island.loop."):
            rural_local_routes += 1
            _check(_all_type(segments, &"dirt"), "rural alternate/local route is dirt: %s" % route_id)
            _check(_route_endpoints_are_rural_hamlets(plan, segments), "dirt route serves rural hamlets only: %s" % route_id)
        else:
            rural_tree_routes += 1
            _check(_all_type(segments, &"gravel"), "rural settlement-tree route is gravel: %s" % route_id)
            _check(_route_endpoints_are_rural_hamlets(plan, segments), "gravel secondary route serves rural hamlets only: %s" % route_id)

    _check(gateway_routes == 4, "four island gateway routes remain")
    _check(major_gateway_routes == 2, "exactly two opposite gateway routes form the sparse freeway axis")
    _check(ordinary_gateway_routes == 2, "the perpendicular gateway pair remains two-lane")
    _check(rural_tree_routes > 0, "gravel secondary network exists")
    _check(rural_local_routes > 0, "dirt rural/local routes exist")
    _check(int(type_counts[&"four_lane"]) > 0, "four-lane freeway segments exist")
    _check(int(type_counts[&"two_lane"]) > int(type_counts[&"four_lane"]), "four-lane segments are sparse relative to ordinary paved roads")
    _check(int(type_counts[&"gravel"]) > 0, "gravel does not disappear")
...
func _routes(plan: Variant) -> Dictionary:
    ...
    for road: Dictionary in plan.road_segments:
        ...
        _check(not route_id.is_empty(), "every road segment has a route id")
...
func _check(condition: bool, message: String) -> void:
    ...
    push_error("PROMPT_ROAD_HIERARCHY_REALISM_FAIL: %s" % message)

func _finish() -> void:
    if failures.is_empty():
        print("PROMPT_ROAD_HIERARCHY_REALISM_OK")
        quit(0)
        return
    for failure: String in failures:
        push_error("PROMPT_ROAD_HIERARCHY_REALISM_FAIL: %s" % failure)
    quit(1)