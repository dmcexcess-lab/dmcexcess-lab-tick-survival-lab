...
func _initialize() -> void:
    ...
    _check(profiles.movement_ticks(Profiles.SKATEBOARD) == 2, "skateboard two-cell move costs two ticks", errors)
    _check(profiles.movement_ticks(Profiles.BICYCLE) == 2, "bicycle three-cell move costs two ticks", errors)
    for motor_kind: StringName in [Profiles.MOTORCYCLE, Profiles.CAR, Profiles.TRUCK]:
        _check(profiles.movement_ticks(motor_kind) == 1, "%s crosses three cells in one tick" % String(motor_kind), errors)
        _check(profiles.full_tank_range_cells(motor_kind) >= Profiles.ISLAND_CROSSING_CELLS, "%s full tank crosses the island" % String(motor_kind), errors)
    _check(profiles.movement_ticks(Profiles.BICYCLE) > profiles.movement_ticks(Profiles.CAR), "bicycle is slower than motor vehicles", errors)
    ...
    _check(not profiles.has_brake(Profiles.SKATEBOARD), "skateboard is the only brakeless vehicle", errors)
    _check(not profiles.requires_stop_before_reverse(Profiles.SKATEBOARD), "skateboard may reverse without braking", errors)
    _check(not profiles.requires_stop_before_exit(Profiles.SKATEBOARD), "skateboard may dismount while moving", errors)
    for braked_kind: StringName in [Profiles.BICYCLE, Profiles.MOTORCYCLE, Profiles.CAR, Profiles.TRUCK]:
        _check(profiles.has_brake(braked_kind), "%s retains a brake" % String(braked_kind), errors)
        _check(profiles.requires_stop_before_reverse(braked_kind), "%s still requires braking before reverse" % String(braked_kind), errors)
        _check(profiles.requires_stop_before_exit(braked_kind), "%s still requires stopping before dismount" % String(braked_kind), errors)
    ...
    var brake_button := controls.find_child("BrakeButton", true, false) as Button
    _check(controls.find_child("ReverseButton", true, false) is Button, "real vehicle panel exposes ReverseButton", errors)
    _check(brake_button != null, "braked vehicles retain BrakeButton", errors)
    controls.call("_apply_vehicle_capabilities", Profiles.SKATEBOARD)
    _check(brake_button != null and not brake_button.visible, "skateboard hides the nonexistent brake control", errors)
    controls.call("_apply_vehicle_capabilities", Profiles.CAR)
    _check(brake_button != null and brake_button.visible, "car restores the brake control", errors)
    controls.queue_free()
