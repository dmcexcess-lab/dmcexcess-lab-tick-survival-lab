...
func _run() -> void:
    ...
    if vehicle_panel != null:
        _check(not vehicle_panel.visible, "vehicle detail panel is hidden while on foot")
        _check(vehicle_panel.find_child("EnterButton", true, false) == null, "vehicle detail panel no longer duplicates ENTER")
