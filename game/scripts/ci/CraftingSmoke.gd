...
func _setup() -> void:
    ...
    crafting = CraftingActionService.new(world, world_mut, hands, hand_mut, inventory, inventory_mut, kernel, recipes, plans)
    _check(plans.is_ready() and crafting.is_ready(), "crafting planner and action service are ready")

func _test_catalog_contract() -> void:
    _check(recipes.catalog_version() == 1 and recipes.recipe_ids().size() == 5, "Candidate 001 exposes five bounded recipes")
...
func _test_personal_possession_and_determinism() -> void:
    ...
    _check(bool(ready.get("ready", false)), "hand recipe becomes ready from personal possession")
...
func _test_timed_transform_and_tool_preservation() -> void:
    ...
    _check(kernel.world_tick() - paper_start == 8, "paper craft spends its recipe WHEN duration")
    ...
    _check(bool(wire_request.get("accepted", false)), "wire craft request accepted")
    kernel.run_until_stop()
...
func _test_cancellation_and_compensation() -> void:
    ...
    _add_carried(&"item.office.scissors", "item.tool.scissors.test")
...
func _test_workstation_and_multistage_chain() -> void:
    ...
    _check(bool(station_plan.get("ready", false)), "multi-stage crafted components plus real tools and reachable workbench satisfy toolkit recipe")
