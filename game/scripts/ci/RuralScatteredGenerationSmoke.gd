...
func _test_canonical_site(
    ...
    _check(_water_service_constraint_valid(request), "%s carries decentralized groundwater service intent only" % site_id)
    ...
    _check(_constraint_count(request, &"potable_water", &"facility") == 0, "%s invents no private well facility" % site_id)
    ...
    _check(_facility_reservation_count(plan) == 0, "%s produces no fake utility facility reservation" % site_id)
    _check(_ordinary_properties_avoid_reservations(plan), "%s ordinary parcels/buildings avoid blocking upstream corridors" % site_id)
...
func _water_service_constraint_valid(request: AreaGenerationRequest) -> bool:
    ...
    for constraint: Dictionary in request.inherited_planning_constraints:
        ...
        if StringName(constraint.get("reservation_role", &"")) != &"service":
            continue
        if StringName(constraint.get("kind", &"")) != &"decentralized_source":
            return false
        if StringName(constraint.get("service_mode", &"")) != &"decentralized_source":
            return false
        if StringName(constraint.get("source_type", &"")) != &"groundwater":
            return false
        found += 1
    return found == 1
