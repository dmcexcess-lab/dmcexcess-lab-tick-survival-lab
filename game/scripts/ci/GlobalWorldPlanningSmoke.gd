...
func _initialize() -> void:
    ...
    _test_global_candidate(planner, validator, power_validator, request, plan)
...
func _test_global_candidate(
    ...
    _check(request.is_valid(), "Slice 004 request is valid")
    _check(plan.is_generated(), "Slice 004 power-aware global world plan generates")
    ...
    _check(bool(power_validator.validate(request, plan).get("ok", false)), "Slice 004 passes independent power validation")
    _check(plan.profile_version == 4, "temperate.rural.region v4 is recorded")
    ...
    _check(_count_power_kind(plan, &"settlement_service") == 5, "all five settlements have service nodes")
    _check(not plan.power_segments.is_empty(), "regional feeder network contains real segments")
    _check(_all_settlements_have_service(plan), "every settlement has exactly one service node at its center")
    _check(_power_ingress_is_boundary(plan), "regional power ingress is on a world boundary gateway")
    _check(_all_power_nodes_reachable(plan), "all power nodes are reachable from the regional ingress")

    var replay: GeneratedGlobalWorldPlan = planner.generate(GlobalFixtureClass.request(GlobalFixtureClass.SEED))
    _check(replay.is_generated() and replay.signature() == plan.signature(), "same seed replays geography/hydrology/roads/power identically")
    ...
    if alternate.is_generated():
        _check(bool(validator.validate(GlobalFixtureClass.request(GlobalFixtureClass.SEED + 1), alternate).get("ok", false)), "alternate seed still passes base global validation")
        _check(bool(power_validator.validate(GlobalFixtureClass.request(GlobalFixtureClass.SEED + 1), alternate).get("ok", false)), "alternate seed still passes power validation")
        _check(_power_signature(alternate) != _power_signature(plan), "alternate seed changes legal power infrastructure")
...
func _test_system20_projection(projector: System20AreaRequestProjector, plan: GeneratedGlobalWorldPlan) -> void:
    ...
    _check(_area_request_signature(projected_request) == _area_request_signature(baseline_request), "Slice 004 leaves Candidate 006 request unchanged")
    ...
    if projected_plan.is_generated() and baseline_plan.is_generated():
        _check(projected_plan.signature() == baseline_plan.signature(), "global power planning does not change Candidate 006 semantic output")
...
func _test_adjacent_road_projection(projector: System20AreaRequestProjector, plan: GeneratedGlobalWorldPlan) -> void:
    ...
    for bounds: Rect2i in windows:
        ...
        _check(bool(result.get("ok", false)), "adjacent road projection remains valid after Slice 004")
...
func _test_power_projection(projector: System20AreaRequestProjector, plan: GeneratedGlobalWorldPlan) -> void:
    ...
    _check(found_crossroads_service, "Candidate 006 exposes its global settlement service node")
...
func _all_settlements_have_service(plan: GeneratedGlobalWorldPlan) -> bool:
