extends RefCounted
class_name TacticalLighting

# Tick Survival Lab adaptation of the reusable physical-lighting rules from
# First Fire's FFTacticalLighting.gd. This module deliberately has no inventory,
# combat, UI, camp, or Game singleton dependency. Equipment can feed a light
# profile into item_contribution() later without lighting owning item data.
...
static func make_source(pos: Vector2i, kind: String, seed_value: int = 0, requires_power: bool = true) -> Dictionary:
...
static func window_daylight_contribution(window_pos: Vector2i, cell: Vector2i) -> float:
    var distance: float = Vector2(cell - window_pos).length()
    if distance > 4.5:
        return 0.0
    return clampf(0.92 * pow(1.0 - distance / 4.5, 0.65), 0.0, 0.92)

static func item_contribution(origin: Vector2i, facing: Vector2i, cell: Vector2i, profile: Dictionary) -> float:
    if profile.is_empty():
        return 0.0
    var light_kind: String = str(profile.get("light", "cone"))
    var diff: Vector2 = Vector2(cell - origin)
    var distance: float = diff.length()
    var max_range: float = maxf(1.0, float(profile.get("light_range", 8.0)))
    if distance <= 0.01:
        return minf(1.0, float(profile.get("light_strength", 1.0)))
    if distance > max_range:
        return 0.0
    var range_factor: float = clampf(1.0 - distance / max_range, 0.0, 1.0)
    var strength: float = float(profile.get("light_strength", 1.0))
    if light_kind == "radial":
        return clampf(strength * pow(range_factor, 0.72), 0.0, 1.0)
    var min_dot: float = clampf(float(profile.get("light_spread", 0.52)), -1.0, 0.99)
...
static func visible_at_distance(light_level: float, distance: int, max_range: int) -> bool:
    ...
    if distance > max_range:
        return false
    var required: float = 0.11 + float(maxi(0, distance - 2)) * 0.085
    if distance == max_range:
        required += 0.06
