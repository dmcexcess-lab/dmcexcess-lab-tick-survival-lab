...
func has_profile(kind: StringName) -> bool:
    return _profiles.has(kind)

func profile(kind: StringName) -> Dictionary:
    return Dictionary(_profiles.get(kind, {})).duplicate(true)

func kinds() -> Array[StringName]:
    return [SKATEBOARD, BICYCLE, MOTORCYCLE, CAR, TRUCK]
...
func kind_for_semantic(semantic_type: StringName) -> StringName:
    ...
    if not text.begins_with(SEMANTIC_PREFIX):
        return &""
    var kind := StringName(text.trim_prefix(SEMANTIC_PREFIX))
    return kind if has_profile(kind) else &""

func is_motorized(kind: StringName) -> bool:
    return bool(profile(kind).get("motorized", false))

func movement_cells(kind: StringName) -> int:
    return int(profile(kind).get("movement_cells", 1))

func max_fuel(kind: StringName) -> int:
    return int(profile(kind).get("max_fuel", 0))

func fuel_per_move(kind: StringName) -> int:
    return int(profile(kind).get("fuel_per_move", 0))

func cargo_grams(kind: StringName) -> int:
    return int(profile(kind).get("cargo_grams", 0))

func hotwire_difficulty(kind: StringName) -> int:
    return int(profile(kind).get("hotwire_difficulty", 0))
...
func _profile(
    ...
    fuel_per_move: int,
    cargo_grams: int,
    fatigue_per_move: int,
    width: int,
    height: int,
    hotwire_difficulty: int,
    ...
    return {
        ...
        "cargo_grams": cargo_grams,
        "fatigue_per_move": fatigue_per_move,
        "width": width,
        "height": height,
        "hotwire_difficulty": hotwire_difficulty,
