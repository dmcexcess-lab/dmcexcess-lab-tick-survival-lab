...
const ISLAND_CROSSING_CELLS: int = 3072

var _profiles: Dictionary = {}

func _init() -> void:
    _profiles = {
        SKATEBOARD: _profile(SKATEBOARD, 2, 2, false, 0, 0, 0, 0, 1, 1, 0, 0, false, false, false),
        BICYCLE: _profile(BICYCLE, 3, 2, false, 0, 0, 6000, 1, 1, 2, 0, 1, true, true, true),
        MOTORCYCLE: _profile(MOTORCYCLE, 3, 1, true, 1400, 1, 12000, 0, 1, 2, 2, 2, true, true, true),
        CAR: _profile(CAR, 3, 1, true, 2800, 2, 70000, 0, 1, 3, 4, 4, true, true, true),
        TRUCK: _profile(TRUCK, 3, 1, true, 4200, 3, 140000, 0, 2, 3, 5, 5, true, true, true),
    }
...
func movement_ticks(kind: StringName) -> int: return int(profile(kind).get("movement_ticks", 1))
...
func has_brake(kind: StringName) -> bool: return bool(profile(kind).get("has_brake", true))
func requires_stop_before_reverse(kind: StringName) -> bool: return bool(profile(kind).get("requires_stop_before_reverse", true))
func requires_stop_before_exit(kind: StringName) -> bool: return bool(profile(kind).get("requires_stop_before_exit", true))

func full_tank_range_cells(kind: StringName) -> int:
    if not is_motorized(kind):
        return 0
    var burn := fuel_per_move(kind)
    if burn <= 0:
        return 0
    return (max_fuel(kind) / burn) * movement_cells(kind)
...
func _profile(kind: StringName, movement_cells: int, movement_ticks: int, motorized: bool, max_fuel: int, fuel_per_move: int, cargo_grams: int, fatigue_per_move: int, width: int, height: int, hotwire_difficulty: int, repair_difficulty: int, has_brake_value: bool, stop_before_reverse: bool, stop_before_exit: bool) -> Dictionary:
    return {
        "kind": kind, "movement_cells": movement_cells, "movement_ticks": maxi(1, movement_ticks), "motorized": motorized,
        ...
        "has_brake": has_brake_value, "requires_stop_before_reverse": stop_before_reverse,
        "requires_stop_before_exit": stop_before_exit,
    }
