...
func is_ready() -> bool:
    return _world != null \
        ...
        and _physical != null \
        and _recipes != null \
        and _crafted_items != null \
        and _workstations != null \
        and _freshness != null \
...
func query(actor_id: String, recipe_id: StringName, workstation_id: String = "") -> Dictionary:
    if not is_ready():
        return _result(Status.UNKNOWN, actor_id, recipe_id, "crafting_plan_not_ready")
    var actor: String = actor_id.strip_edges()
    var recipe_value: CraftingRecipe = _recipes.recipe(recipe_id)
    if recipe_value == null:
        return _result(Status.UNKNOWN, actor, recipe_id, "recipe_unknown")
    var common: Dictionary = _validate_common(actor, recipe_value, workstation_id)
    if int(common.get("status", Status.UNKNOWN)) != Status.READY:
        return common

    var carry_result: Dictionary = common.get("carry", {})
    var personal_ids: Array = carry_result.get("item_ids", [])
    var by_semantic: Dictionary = _semantic_index(personal_ids)
    var used: Dictionary = {}
    ...
    for item_id: Variant in consumed_ids:
        used[String(item_id)] = true
    ...
    var tool_ids: Array = selected_tools.get("item_ids", [])
...
func validate_exact(
    ...
    if not is_ready():
        return _result(Status.UNKNOWN, actor_id, recipe_id, "crafting_plan_not_ready")
    var actor: String = actor_id.strip_edges()
    if expected_catalog_version != _recipes.catalog_version():
        return _result(Status.BLOCKED, actor, recipe_id, "recipe_catalog_version_stale")
    var recipe_value: CraftingRecipe = _recipes.recipe(recipe_id)
    if recipe_value == null:
        return _result(Status.UNKNOWN, actor, recipe_id, "recipe_unknown")
    var common: Dictionary = _validate_common(actor, recipe_value, workstation_id)
    ...
    for item_value: Variant in carry_result.get("item_ids", []):
        personal_set[String(item_value)] = true
    ...
    if consumed_ids.size() != recipe_value.consumed_entity_count():
        return _result(Status.BLOCKED, actor, recipe_id, "consumed_item_count_stale")
    if tool_ids.size() != recipe_value.tool_entity_count():
        return _result(Status.BLOCKED, actor, recipe_id, "tool_item_count_stale")

    var consumed_set: Dictionary = {}
    for item_value: Variant in consumed_ids:
        var item_id: String = String(item_value)
        if not personal_set.has(item_id):
            return _result(Status.BLOCKED, actor, recipe_id, "input_no_longer_personal:%s" % item_id)
        if _containment.has_container(item_id):
            return _result(Status.BLOCKED, actor, recipe_id, "container_item_cannot_be_consumed:%s" % item_id)
        consumed_set[item_id] = true
    for item_value: Variant in tool_ids:
        var item_id: String = String(item_value)
        if consumed_set.has(item_id):
            return _result(Status.BLOCKED, actor, recipe_id, "tool_overlaps_consumed_input")
        if not personal_set.has(item_id):
            return _result(Status.BLOCKED, actor, recipe_id, "tool_no_longer_personal:%s" % item_id)

    if not _ids_match_requirements(consumed_ids, recipe_value.consumed_inputs):
        return _result(Status.BLOCKED, actor, recipe_id, "consumed_item_semantics_stale")
    if not _ids_match_requirements(tool_ids, recipe_value.required_tools):
        return _result(Status.BLOCKED, actor, recipe_id, "tool_item_semantics_stale")
    return _finalize_ready(actor, recipe_value, workstation_id.strip_edges(), consumed_ids, tool_ids, carry_result)
...
func _validate_common(actor: String, recipe_value: CraftingRecipe, workstation_id: String) -> Dictionary:
    if actor.is_empty() or not _world.has_entity(actor):
        return _result(Status.BLOCKED, actor, recipe_value.recipe_id, "actor_missing")
    ...
    if actor_entity == null or String(actor_entity.semantic_type) != "actor.survivor":
        return _result(Status.BLOCKED, actor, recipe_value.recipe_id, "not_survivor")
    if not _hands.has_actor(actor) or not _containment.has_container(actor):
        return _result(Status.UNKNOWN, actor, recipe_value.recipe_id, "personal_possession_unclassified")
    if not _recipe_semantics_safe(recipe_value):
        return _result(Status.UNKNOWN, actor, recipe_value.recipe_id, "recipe_semantic_contract_invalid")
    var workstation_reason: String = _workstation_reason(actor, recipe_value, workstation_id.strip_edges())
    if not workstation_reason.is_empty():
        return _result(Status.BLOCKED, actor, recipe_value.recipe_id, workstation_reason)
...
func _recipe_semantics_safe(recipe_value: CraftingRecipe) -> bool:
    if recipe_value == null or not recipe_value.is_valid():
        return false
    for requirement: Dictionary in recipe_value.consumed_inputs:
        var semantic := StringName(requirement.get("semantic_type", &""))
        if _freshness.has_profile(semantic) or not _physical.has_profile(semantic):
            return false
    ...
    for requirement: Dictionary in recipe_value.outputs:
        var semantic := StringName(requirement.get("semantic_type", &""))
        if _freshness.has_profile(semantic) or not _crafted_items.has_item(semantic) or not _physical.has_profile(semantic):
            return false
    return true

func _workstation_reason(actor: String, recipe_value: CraftingRecipe, workstation_id: String) -> String:
    if String(recipe_value.workstation_capability).is_empty():
        return ""
    if workstation_id.is_empty() or not _world.has_entity(workstation_id):
    ...
    if entity == null or placement == null or placement.channel != Layers.Channel.OBJECT:
        return "workstation_unavailable"
    if not _workstations.supports(entity.semantic_type, recipe_value.workstation_capability):
        return "workstation_capability_mismatch"
    if not _reach.target_reachable(actor, workstation_id, WorldInteractionReachQuery.CONTACT_FORWARD):
        return "workstation_out_of_reach"
    return ""
...
func _select_requirements(requirements: Array[Dictionary], by_semantic: Dictionary, used: Dictionary, reject_containers: bool) -> Dictionary:
    ...
    for requirement: Dictionary in requirements:
        ...
        for item_value: Variant in candidates:
            ...
            if used.has(item_id):
                continue
            if reject_containers and _containment.has_container(item_id):
                continue
...
func _ids_match_requirements(item_ids: Array, requirements: Array[Dictionary]) -> bool:
    ...
    for requirement: Dictionary in requirements:
        expected[String(requirement.get("semantic_type", ""))] = int(requirement.get("count", 0))
...
func _finalize_ready(
    actor: String,
    recipe_value: CraftingRecipe,
    workstation_id: String,
    consumed_ids: Array,
    tool_ids: Array,
    carry_result: Dictionary
) -> Dictionary:
    var consumed_weight: int = 0
    for item_value: Variant in consumed_ids:
        var item_id: String = String(item_value)
        if not _world.has_entity(item_id):
            return _result(Status.BLOCKED, actor, recipe_value.recipe_id, "input_missing:%s" % item_id)
        var entity: WorldEntityRecord = _world.entity(item_id)
        var weight: int = -1 if entity == null else _physical.weight_grams(entity.semantic_type)
        if weight <= 0:
            return _result(Status.UNKNOWN, actor, recipe_value.recipe_id, "input_weight_unknown:%s" % item_id)
    ...
    for requirement: Dictionary in recipe_value.outputs:
        ...
        if weight <= 0:
            return _result(Status.UNKNOWN, actor, recipe_value.recipe_id, "output_weight_unknown:%s" % String(semantic))
        for _index in range(count):
            output_semantics.append(semantic)
            output_weight += weight
    if output_weight > consumed_weight:
        return _result(Status.UNKNOWN, actor, recipe_value.recipe_id, "recipe_creates_mass")

    var current_weight: int = int(carry_result.get("weight_grams", -1))
    var hard_limit: int = int(carry_result.get("hard_limit_grams", -1))
    if current_weight < 0 or hard_limit <= 0:
        return _result(Status.UNKNOWN, actor, recipe_value.recipe_id, "carry_projection_unknown")
    var projected: int = current_weight - consumed_weight + output_weight
    if projected > hard_limit:
        return _result(Status.BLOCKED, actor, recipe_value.recipe_id, "absolute_carry_limit_exceeded")
...
static func _result(status_value: int, actor_id: String, recipe_id: StringName, reason: String) -> Dictionary:
    return {
        "status": status_value,
        "ready": status_value == Status.READY,
        ...
        "current_carry_grams": 0,
        "projected_carry_grams": 0,
        "hard_limit_grams": 0,
    }
