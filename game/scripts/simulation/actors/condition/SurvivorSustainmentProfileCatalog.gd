...
## System 34 food/drink effects for existing real item semantics.
## Cooking-required raw staples are deliberately absent until their real prepared forms exist.
...
func _add(
    ...
    var key: String = String(semantic_type)
    if key.is_empty() or action_kind not in [&"eat", &"drink"] or duration_ticks < 1:
        return
    _profiles[key] = {
