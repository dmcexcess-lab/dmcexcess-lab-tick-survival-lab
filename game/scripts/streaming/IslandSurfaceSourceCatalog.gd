...
func source_handle_for_id(source_id: String) -> Dictionary:
    ...
    if value.is_empty():
        return {}
    return {"source_kind": SOURCE_KIND, "source_id": source_id, "source_key": String(value.get("source_key", "")), "bounds": value.get("bounds", Rect2i())}
    ...
    for source: Dictionary in _sources:
        ...
        for query: Rect2i in bounds_list:
            if _overlap(rect, query):
                result.append({"source_kind": SOURCE_KIND, "source_id": String(source.get("source_id", "")), "source_key": String(source.get("source_key", "")), "bounds": rect})
                break
    return result
...
func validate_source_bounds(plan: GeneratedGlobalWorldPlan) -> Dictionary:
    ...
    for first_index: int in range(_sources.size()):
        var first: Rect2i = _sources[first_index].get("bounds", Rect2i())
        if not _rect_inside(plan.bounds, first): return {"ok": false, "failure_reason": "island_surface_source_out_of_bounds"}
        for site: Dictionary in plan.area_sites:
            if _overlap(first, site.get("bounds", Rect2i())): return {"ok": false, "failure_reason": "island_surface_source_overlaps_site"}
        for second_index: int in range(first_index + 1, _sources.size()):
            if _overlap(first, _sources[second_index].get("bounds", Rect2i())): return {"ok": false, "failure_reason": "island_surface_sources_overlap"}
    return {"ok": true, "failure_reason": ""}

func _subtract_all(pieces: Array[Rect2i], exclusion: Rect2i) -> Array[Rect2i]:
    var result: Array[Rect2i] = []
    for piece: Rect2i in pieces: result.append_array(_subtract(piece, exclusion))
    return result

func _subtract(source: Rect2i, exclusion: Rect2i) -> Array[Rect2i]:
    var overlap_rect: Rect2i = _intersection(source, exclusion)
    if overlap_rect.size.x <= 0 or overlap_rect.size.y <= 0: return [source]
    var result: Array[Rect2i] = []
    var source_end: Vector2i = source.position + source.size
    var overlap_end: Vector2i = overlap_rect.position + overlap_rect.size
    if overlap_rect.position.y > source.position.y: result.append(Rect2i(source.position, Vector2i(source.size.x, overlap_rect.position.y - source.position.y)))
    if overlap_end.y < source_end.y: result.append(Rect2i(Vector2i(source.position.x, overlap_end.y), Vector2i(source.size.x, source_end.y - overlap_end.y)))
    if overlap_rect.position.x > source.position.x: result.append(Rect2i(Vector2i(source.position.x, overlap_rect.position.y), Vector2i(overlap_rect.position.x - source.position.x, overlap_rect.size.y)))
    if overlap_end.x < source_end.x: result.append(Rect2i(Vector2i(overlap_end.x, overlap_rect.position.y), Vector2i(source_end.x - overlap_end.x, overlap_rect.size.y)))
    return result
...
func _intersection(a: Rect2i, b: Rect2i) -> Rect2i:
    ...
    if finish.x <= start.x or finish.y <= start.y: return Rect2i()
