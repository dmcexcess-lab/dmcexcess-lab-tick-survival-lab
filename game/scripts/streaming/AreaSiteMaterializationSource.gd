...
func _init(registry: MaterializationRegistry = null, projector: System20AreaRequestProjector = null, generator: LocalAreaGenerator = null) -> void:
    _registry = registry
...
func source_key_for_site(site_id: String) -> String:
    ...
    return "" if clean.is_empty() else "system20_area_site:%s" % clean
...
func source_handle_for_site(global_plan: GeneratedGlobalWorldPlan, site_id: String) -> Dictionary:
    ...
    if clean_site_id.is_empty() or site.is_empty(): return {}
    var bounds: Rect2i = site.get("bounds", Rect2i())
    var source_key: String = source_key_for_site(clean_site_id)
    if source_key.is_empty() or bounds.size.x <= 0 or bounds.size.y <= 0: return {}
    return {"source_kind": SOURCE_KIND, "source_id": clean_site_id, "source_key": source_key, "bounds": bounds}

func source_handles_intersecting(global_plan: GeneratedGlobalWorldPlan, bounds_list: Array[Rect2i]) -> Array[Dictionary]:
    var result: Array[Dictionary] = []
    if not bool(validate_source_bounds(global_plan).get("ok", false)): return result
    for site_id: String in site_ids_intersecting(global_plan, bounds_list):
        ...
        if not handle.is_empty(): result.append(handle)
    result.sort_custom(func(a: Dictionary, b: Dictionary) -> bool: return String(a.get("source_key", "")) < String(b.get("source_key", "")))
    return result

func validate_source_bounds(global_plan: GeneratedGlobalWorldPlan) -> Dictionary:
    if global_plan == _validated_plan and not _validation_result.is_empty(): return _validation_result.duplicate(true)
    var result: Dictionary = {"ok": true, "failure_reason": ""}
    if global_plan == null or not global_plan.is_generated():
        result = {"ok": false, "failure_reason": "invalid_global_plan"}
    else:
        ...
        for index: int in range(ordered.size()):
            ...
            if site_id.is_empty() or bounds.size.x <= 0 or bounds.size.y <= 0 or not _rect_inside(global_plan.bounds, bounds):
                result = {"ok": false, "failure_reason": "area_site_bounds_invalid:%s" % site_id}
                break
            for other_index: int in range(index + 1, ordered.size()):
                var other_bounds: Rect2i = ordered[other_index].get("bounds", Rect2i())
                if _rects_overlap_positive(bounds, other_bounds):
                    result = {"ok": false, "failure_reason": "area_site_bounds_overlap:%s:%s" % [site_id, String(ordered[other_index].get("id", ""))]}
                    break
...
func site_ids_intersecting(global_plan: GeneratedGlobalWorldPlan, bounds_list: Array[Rect2i]) -> Array[String]:
    ...
    if global_plan == null or not global_plan.is_generated() or bounds_list.is_empty(): return result
    for site: Dictionary in global_plan.area_sites:
        var site_id: String = String(site.get("id", "")).strip_edges()
        var site_bounds: Rect2i = site.get("bounds", Rect2i())
        if site_id.is_empty() or site_bounds.size.x <= 0 or site_bounds.size.y <= 0: continue
...
func prepare(global_plan: GeneratedGlobalWorldPlan, site_id: String) -> Dictionary:
    if not is_ready() or global_plan == null or not global_plan.is_generated(): return _failure("invalid_materialization_source_input")
    var clean_site_id: String = site_id.strip_edges()
    if clean_site_id.is_empty(): return _failure("area_site_id_invalid")
    var site: Dictionary = _site_by_id(global_plan.area_sites, clean_site_id)
    if site.is_empty(): return _failure("area_site_missing")
    var source_key: String = source_key_for_site(clean_site_id)
    if source_key.is_empty(): return _failure("source_key_invalid")
    if _registry.has_source(source_key): return _failure("source_already_materialized")
    var projected: Dictionary = _projector.project_site(global_plan, clean_site_id)
    if not bool(projected.get("ok", false)): return _failure("site_projection_failed:%s" % String(projected.get("failure_reason", "unknown")))
    var request: AreaGenerationRequest = projected.get("request") as AreaGenerationRequest
    if request == null or not request.is_valid(): return _failure("site_projection_request_invalid")
    var plan: GeneratedAreaPlan = _generator.generate(request)
    if plan == null or not plan.is_generated(): return _failure("site_local_generation_failed:%s" % ("null_plan" if plan == null else plan.failure_reason))
    var site_bounds: Rect2i = site.get("bounds", Rect2i())
    var site_seed: int = int(site.get("seed", 0))
    if request.area_id != clean_site_id or plan.area_id != clean_site_id: return _failure("prepared_area_identity_mismatch")
    if request.bounds != site_bounds or plan.bounds != site_bounds: return _failure("prepared_area_bounds_mismatch")
    if request.seed != site_seed or plan.seed != site_seed: return _failure("prepared_area_seed_mismatch")
    return {"ok": true, "failure_reason": "", "source_key": source_key, "source_kind": SOURCE_KIND, "source_id": clean_site_id, "bounds": site_bounds, "source_seed": site_seed, "request": request, "plan": plan, "plan_signature": plan.signature()}

func _failure(reason: String) -> Dictionary:
    return {"ok": false, "failure_reason": reason, "source_key": "", "source_kind": SOURCE_KIND, "source_id": "", "request": null, "plan": null}
...
func _rects_overlap_positive(a: Rect2i, b: Rect2i) -> bool:
    if a.size.x <= 0 or a.size.y <= 0 or b.size.x <= 0 or b.size.y <= 0: return false
