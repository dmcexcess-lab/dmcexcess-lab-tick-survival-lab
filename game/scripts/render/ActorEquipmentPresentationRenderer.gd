...
func configure(world: WorldState, equipment: ActorHandEquipmentState) -> bool:
    if world == null or equipment == null: return false
    _world = world
    _equipment = equipment
    var changed := Callable(self, "_on_equipment_changed")
    if not _equipment.equipment_assignment_changed.is_connected(changed): _equipment.equipment_assignment_changed.connect(changed)
    var reset := Callable(self, "_on_equipment_reset")
    if not _equipment.hand_equipment_reset.is_connected(reset): _equipment.hand_equipment_reset.connect(reset)
    var world_changed := Callable(self, "_on_world_changed")
    if not _world.changed.is_connected(world_changed): _world.changed.connect(world_changed)
    queue_redraw()
    return true

func is_configured() -> bool: return _world != null and _equipment != null

func set_visible_window(origin: Vector2i, size_cells: Vector2i, cell_pixels: float) -> bool:
    if size_cells.x <= 0 or size_cells.y <= 0 or cell_pixels <= 0.0: return false
    _origin = origin
    _size = size_cells
...
func _draw() -> void:
    if not is_configured() or not _view_valid: return
    for actor_id: String in _equipment.actor_ids():
        var placement := _world.placement(actor_id)
        if placement == null or placement.channel != Layers.Channel.ACTOR: continue
        if placement.anchor.x < _origin.x or placement.anchor.y < _origin.y or placement.anchor.x >= _origin.x + _size.x or placement.anchor.y >= _origin.y + _size.y: continue
...
func _draw_apparel(actor_id: String, center: Vector2, facing: int) -> void:
    ...
    if torso != &"":
        ...
        if torso == &"item.apparel.t_shirt": torso_color = Color(0.58, 0.58, 0.54, 0.90)
        elif torso == &"item.apparel.hoodie": torso_color = Color(0.24, 0.31, 0.39, 0.92)
        draw_rect(Rect2(center + Vector2(-s * 0.22, -s * 0.05), Vector2(s * 0.44, s * 0.32)), torso_color, true)
    if legs != &"":
        var leg_color := Color(0.18, 0.24, 0.34, 0.92) if legs == &"item.apparel.jeans" else Color(0.30, 0.31, 0.22, 0.92)
        draw_rect(Rect2(center + Vector2(-s * 0.19, s * 0.22), Vector2(s * 0.38, s * 0.24)), leg_color, true)
    if feet != &"":
        var foot_color := Color(0.16, 0.12, 0.09, 0.96) if feet == &"item.apparel.work_boots" else Color(0.27, 0.27, 0.27, 0.96)
    ...
    if head != &"":
        ...
        if head == &"item.apparel.baseball_cap":
            var forward := Vector2(FacingRules.vector(facing))
...
func _on_equipment_changed(_actor_id: String, _slot: int, _old: String, _new: String, _version: int) -> void: queue_redraw()
func _on_equipment_reset() -> void: queue_redraw()
func _on_world_changed(_change: Variant) -> void: queue_redraw()
