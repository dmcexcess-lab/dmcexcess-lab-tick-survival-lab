...
var _world_interaction_state: WorldInteractableStateRenderer = null
...
func configure(world: WorldState, art_catalog: ArtCatalog, door_state: DoorStateStore, controlled_actor_id: String) -> bool:
    ...
    if not _actors.set_controlled_actor_id(controlled_actor_id):
        return false
    var equipment_state := _find_owner_equipment_state()
    if equipment_state != null and not _equipment.configure(world, equipment_state):
        return false
    _configured = true
    return true
...
func configure_actor_equipment(world: WorldState, equipment_state: ActorHandEquipmentState) -> bool:
    _ensure_layers()
    if not _equipment.configure(world, equipment_state):
        return false
    if _ground.has_valid_view():
        return _equipment.set_visible_window(_ground.visible_origin(), _ground.visible_size(), _ground.cell_pixels())
    return true
...
func set_visible_window(origin: Vector2i, size_cells: Vector2i, cell_pixels: float) -> bool:
    ...
    var vehicle_ok := true
    if _vehicles.is_configured():
        vehicle_ok = _vehicles.set_visible_window(origin, size_cells, cell_pixels)
    var equipment_ok := true
    if _equipment.is_configured():
        equipment_ok = _equipment.set_visible_window(origin, size_cells, cell_pixels)
    ...
    if _world_interaction_state.is_configured():
        world_state_ok = _world_interaction_state.set_visible_window(origin, size_cells, cell_pixels)
    ...
    return _ground.set_visible_window(origin, size_cells, cell_pixels) \
        ...
        and power_ok and vehicle_ok and equipment_ok and lighting_ok and weather_ok and world_state_ok and interaction_ok \
...
func _find_owner_equipment_state() -> ActorHandEquipmentState:
    var owner_node := get_parent()
    if owner_node == null:
        return null
    for descriptor: Dictionary in owner_node.get_property_list():
        if String(descriptor.get("name", "")) == "_hand_state":
            var value: Variant = owner_node.get("_hand_state")
            return value as ActorHandEquipmentState
    return null

func _ensure_layers() -> void:
    if _ground != null:
        return
    _ground = GroundRendererClass.new(); _ground.name = "Ground"; _ground.z_index = 0; add_child(_ground)
    _structures = StructureRendererClass.new(); _structures.name = "Structures"; _structures.z_index = 10; add_child(_structures)
