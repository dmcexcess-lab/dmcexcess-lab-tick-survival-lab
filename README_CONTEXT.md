...
Durable design references: `DESIGN.md`, `ROADMAP.md`, `WORLD_GENERATION.md`, `WORLD_NAVIGATION_AUDIT.md`, `ART_VOCABULARY.md`, `MINI_WORLD_STREETSCAPE_DESIGN.md`, `EXTRACTION_RAID_DESIGN.md`, and `FIRST_FIRE_REUSE.md`.

**`EXTRACTION_RAID_DESIGN.md` supersedes the seamless adjacent-region travel portions of `MINI_WORLD_STREETSCAPE_DESIGN.md`.** The v5 generator, streetscape, building-family, art and performance rules remain valid.

## Current milestone / stage

Milestone 0.1 authoritative tick movement and 0.2 action execution are complete. 0.3A visual perception is complete. 0.3B weather foundation is functionally present. The current gameplay shell is now an **extraction loop**:
...
- `ProceduralRegionGenerator.gd` — deterministic v4 64×64 physical baseline: biomes, roads, parcels, structures, clutter, lights and exits.
- `MiniRegionGenerator.gd` — generator v5 orchestration; wraps v4 and applies region-focus + streetscape/building coherence.
- `MiniRegionFocusPass.gd` — guarantees enough focus-appropriate shells for commercial/residential/rural/etc. destinations.
- `StreetscapePass.gd` — traffic controls, street furniture, building families, parking repair and door-run sanitation at generation time.
...
- `MapPreview.gd` / `MapPreviewPresentation.gd` / `MiniWorldPresentation.gd` — inherited developer/presentation layers; the seamless region-travel behavior in `MiniWorldPresentation.gd` is no longer the active main-scene behavior.
- `ExtractionWorldPresentation.gd` — active playable presentation harness. It binds the destination map to `ExtractionRaidState`, loads selected raids, makes raid maps view-only while deployed, and converts generated edge exits into extraction points. It owns presentation/orchestration only, not the raid-session truth.
...
A raid seed is derived from world seed + destination's stable site seed + destination coordinate + that destination's visit count.
...
Generated green edge exit cells are now extraction cells, not links to neighboring tactical regions.
...
V5 generation remains active inside every raid.
...
- parking-only legacy commercial parcels are repaired into strip malls;
- traffic lights and stop signs are actually placed;
...
Preferred next gameplay order is:

1. playtest deploy/extract loop;
...
3. `README_SOPS.md`
4. This context file
5. `EXTRACTION_RAID_DESIGN.md`
6. Other durable design docs
