...
1. seal consequences already earned for this timestamp;
2. describe departures/releases, trajectories, contacts, arrival claims and forces from the stable incoming state;
3. resolve physical interaction and exclusive contests without callback/actor-ID initiative;
4. solve occupancy dependencies to a fixed point;
5. publish final spatial truth atomically;
6. apply bodily/condition consequences generated by those contacts;
7. publish terminal outcomes such as incapacitation/death/corpses into the outgoing state.

This is a causal ordering, not an actor initiative ordering. Same-timestamp actors do not take hidden turns.

**Movement consequence:** movement is represented conceptually as a conditional release of the origin plus an arrival claim on the destination. A follower may enter a cell vacated by a successful same-tick departure. If the upstream departure fails, dependent arrivals fail back through the chain.

**Edge conflict:** two ordinary walkers attempting `X -> Y` and `Y -> X` traverse the same physical edge in opposite directions. They meet; they do not pass through each other as a free atomic swap. Both remain in place with `movement_edge_conflict`. This supersedes the earlier provisional rule that reciprocal occupied-cell walks may always swap atomically.

Longer compatible release chains/cycles are not automatically invalid: the conflict is opposing traversal of the same edge, not merely entering a cell that was occupied in `S_t`.

**Commitment/death implication:** an already-earned consequence for the current timestamp is not retroactively erased because damage/death is also generated during that timestamp. Final death/corpse publication belongs after already-committed current-tick consequences have been accounted for. This ordering is canonical design direction; cross-system shove/death wiring remains staged work.
