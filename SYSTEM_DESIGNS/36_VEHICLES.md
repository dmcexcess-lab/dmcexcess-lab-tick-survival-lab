...
Authoritative NORTH footprints are **1×3 cells for cars** and **2×3 cells for trucks**. The same real footprint records drive generated placement, occupancy and movement collision. Rendering centers each sprite over those occupied world cells and applies no class-specific size override. The car sprite uses a matching 1:3 canvas so its art fills the real three-cell length instead of leaving a visually empty cell.
...
- cars use a real **1×3** footprint and trucks use a real **2×3** footprint across generation, occupancy, collision and presentation; no mismatched visual scale/hitbox is allowed;
